...
## 21. Раунд 11 — окружение сцены реагирует на звук

Продолжение «живой» сцены неона: теперь не только эквалайзер, но и всё
окружение реагирует на **реальную** энергию звука
(`AudioVisualizer.energy`, 0..1):

- **Кольца сцены** (`_StageRingsPainter`): подписаны на `AudioVisualizer.energy`
  через `super(repaint: ...)` — «дышат» в такт (радиус ×1.0..1.07, толщина
  +0..0.7, яркость 0.30..0.60). Раньше были полностью статичны.
- **Ambient-свечение** (`_AmbientGlow`): альфа обоих световых пятен усиливается
  от энергии (0.16→0.42 и 0.12→0.38) — фоновое окружение «набухает» на громком.
- **Центральная обложка** (`_CinematicCard`): мягкая пульсация `scale`
  (1.0..1.03) под ритм, через `ValueListenableBuilder` с кешированным `child`
  (тяжёлое поддерево не перестраивается — только трансформ).

Всё сделано на GPU-лёгких операциях (transform/alpha), без `saveLayer`
и без blur — чтобы не вернуть прежние лаги.

### 21.1. Валидация

- `dart format` — файл разбирается без ошибок синтаксиса.
- `dart analyze lib` — проверка выполнена (см. лог сборки/репозитория).

### 21.2. Изменённые файлы раунда 11

```
lib/features/now_playing/cinematic/cinematic_player_body.dart
...
## 23. Раунд 13 — «свечение пола» под живым эквалайзером

Последний штрих сцены: под полосами живого эквалайзера добавлено неоновое
«свечение пола» (`RadialGradient`, центр у нижней кромки), яркость которого
пульсирует от средней громкости (`avg` по полосам, 0.26..0.60).

Это усиливает «живость»: на громком фрагменте сцена подсвечивается снизу.
Рисуется одним `drawRect` с шейдером — дёшево для GPU, без blur/`saveLayer`.

### 23.1. Валидация

- `dart analyze lib/widgets/live_equalizer.dart` → **No issues found!**

### 23.2. Изменённые файлы раунда 13

```
lib/widgets/live_equalizer.dart   (неоновое «свечение пола» от громкости)
```
