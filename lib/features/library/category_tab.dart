...
class _CategoryTabState extends State<CategoryTab> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedGenre != 'Прочее' &&
        !genreCounts.containsKey(_selectedGenre) &&
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  'Веса контекстов (влияют на рекомендации)',
                  style: TextStyle(
                    color: AppTheme.textPrimary,
                          ...
                          children: [
                            ...
                            SliderTheme(
                              ...
                              child: Slider(
                                ...
                                onChanged: (v) =>
                                    player.setCategoryWeight(ctx, v),
                              ),
                            ),
                            Text(
                              player.categoryCriteria(ctx),
                              style: TextStyle(
                                color: AppTheme.textMuted,
                                fontSize: 10,
                                height: 1.2,
                    ...
                    return Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Жанры: $cached/$total ($pct%) в кэше • фоновая догрузка по названию',
                              style: TextStyle(
                          ...
                          TextButton(
                            onPressed: () => player.fetchGenresForVisible(
                              batchSize: 20,
                              force: true,
                            ),
              ...
              if (!_genresMode)
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 14),
                  child: Text(
                    'Категории: тап — фильтр, долгое нажатие на треке — ручная разметка',
              ...
              if (_genresMode)
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 14),
                  child: Text(
                    'Жанры: тап — фильтр, долгое нажатие на треке — ручной выбор жанра',
                    style: TextStyle(color: AppTheme.textMuted, fontSize: 11),
                  ),
                ),
              const SizedBox(height: 8),
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Text(
                      '$_selected: ${tracks.length} из $total треков',
                      style: TextStyle(color: AppTheme.textMuted, fontSize: 12),
                    ),
                    const Spacer(),
                    Text(
                      'фильтр: ${_ctxLabel(_selected)}',
                      style: TextStyle(
                        color: AppTheme.accentLight,
                        fontSize: 11,
                        fontWeight: FontWeight.w600,
              ...
              if (_genresMode)
                Padding(
                  ...
                  child: Row(
                    children: [
                      Text(
                        '$_selectedGenre: ${genreTracks.length} из $total треков',
                        style: TextStyle(
                          color: AppTheme.textMuted,
                          fontSize: 12,
                        ),
                  ...
                  children: [
                    Icon(
                      ...
                      color: AppTheme.textMuted.withValues(
                        alpha: AppTheme.textMuted.a * (0.5),
                      ),
                    ),
                    const SizedBox(height: 12),
                    Text(
                      'Нет треков в категории "${_ctxLabel(_selected)}"',
                      style: TextStyle(color: AppTheme.textMuted),
                    ),
                    const SizedBox(height: 6),
                    Text(
                      'Попробуйте изменить ключевые слова или добавить вручную',
                      textAlign: TextAlign.center,
                      style: TextStyle(
                        color: AppTheme.textMuted.withValues(
                          alpha: AppTheme.textMuted.a * (0.7),
                        ),
                        fontSize: 12,
                      ),
                    ),
              ...
              child: Center(
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      'Нет треков в жанре "$_selectedGenre"',
                      style: TextStyle(color: AppTheme.textMuted),
                    ),
                    const SizedBox(height: 6),
                    Text(
                      'Назначьте жанр вручную долгим нажатием на трек',
                      textAlign: TextAlign.center,
                      style: TextStyle(color: AppTheme.textMuted, fontSize: 12),
                    ),
                  ],
                    ...
                    children: [
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            src == 'manual'
                                ? '${player.primaryGenre(track)} • свой'
                                : '${player.primaryGenre(track)} • $src',
                            style: TextStyle(
                              color: src == 'manual'
                                  ? Colors.white
                                  : AppTheme.textMuted,
                              fontSize: 9,
                              fontWeight: src == 'manual'
                                  ? FontWeight.bold
                                  : FontWeight.normal,
                            ),
                          ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Сейчас: $current ($src) • у трека один основной жанр',
                    style: TextStyle(color: AppTheme.textMuted, fontSize: 11),
                  ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    isManual
                        ? 'Ручной приоритет • авто-определение отключено'
                        : 'Авто: ${current.map(_ctxLabel).join(", ")} — тапните чтобы переопределить',
                    style: TextStyle(color: AppTheme.textMuted, fontSize: 11),
                  ),
                  const SizedBox(height: 14),
                  Wrap(
